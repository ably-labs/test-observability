...
export class UploadsFilterWhereClause {
  ...
  private createClause(subClauses: string[], options: ClauseCreationOptions): string | null {
    if (subClauses.length == 0) {
      return null
    }

    return `${options.includeWhereKeyword ? 'WHERE ' : ''}${subClauses.join(' AND ')}`
  }
  ...
  uploadsAndFailuresClause(options: ClauseCreationOptions): string | null {
    return this.createClause(this.uploadsSubClauses.concat(this.failuresSubClauses), options)
  }
}
